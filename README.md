...
2. **Claim.** mkvid polls `POST /mkvid/claim` (bearer `MKVID_TOKEN`) whenever its render slot is free and gets the next claimable request, or `null`. **Newest set first**: the queue is ordered by the set's date (`set_date`, from the `-YYYY-MM-DD.html` URL slug, else the page's `datePublished` / title), undated sets last, ties by most recently queued — so a DJ's latest show is uploaded before their 2014 back catalogue. The panel's mkvid section lists the waiting line in this order. A request whose set has meanwhile gained a real recording is marked `superseded` and skipped. A claim nobody reports on within `MKVID_CLAIM_TTL_SECONDS` (default 3 h — mkvid died mid-job) is handed out again; three claims and it is `failed`. **At most `MKVID_DAILY_CLAIM_CAP` claims per quota day (midnight Pacific, when the YouTube quota resets; default 2)**: mkvid uploads through the same Google Cloud project as the sync, and a `videos.insert` costs 1 600 of the project's 10 000 daily YouTube quota units, so an unthrottled queue would starve the sync's own playlist inserts (the backlog simply drains at two sets a day). Raise it only with a separate Cloud project for mkvid or a quota increase; the count is `claimed`/`done` rows in `mkvid_requests` with `claimed_at` since midnight Pacific, so a deploy cannot reset it and a request refused before rendering does not use a slot. `MKVID_DAILY_CLAIM_CAP=0` **pauses** the queue (every claim is refused) — it has to be a literal `0`; a blank value means the default. The panel says so in red when it is paused.
