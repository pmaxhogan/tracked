...
Each `/now-playing` call also writes a durable audit record to KV under `np:<epochMs>:<reqId>` (90-day TTL) with the request inputs (`currentSeconds`, `videoDurationSeconds`, title) and the selection it produced. Workers Logs only retains ~3 days, but timestamp/selection bugs are often noticed much later (a wrong "now playing" spotted in an old screenshot). Inspect with `wrangler kv key list --namespace-id <CACHE id> --prefix np:` then `wrangler kv key get`.
