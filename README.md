...
Rechecks run after the new-set window in every sync (daily, 5-minute drain, manual) so a backfill is never starved, capped at 20 per DJ per run, newest sets first. A recheck that keeps failing (3×) is deferred to the next interval rather than abandoned — the video it already has stays put. `playlistItems.delete` costs the same 50 units as an insert; a swap is rare enough that this isn't budgeted, but every removal shows up in the audit row's message.

**Invalidate video cache & resync.** Each DJ row in the panel has an **Invalidate & resync** button, and the list header has one for every DJ. It marks every processed set due *now* (keeping its recorded video, so swaps are detected and old videos removed), gives abandoned sets another chance, drops the cached membership of both the artist playlist and the combined playlist (`yt:plvids:<id>`) so they're re-read from YouTube, and runs one sync. One run rechecks at most 20 sets and, like every other run, stops when the per-tick fetch budget is spent (see **Pacing** below); the 5-minute cron drains the rest — the panel message says how many are still pending. The list-header button is `POST /subscriptions/api/resync` — one server-side pass over every DJ on a single shared budget. Until 2026-09-14 it looped the per-DJ endpoint from the browser, and neither the loop nor the per-DJ endpoints carried a budget, so a "resync all" fetched every set of every DJ flat out; that is what tripped the 1001tracklists rate limit on both ban days.

```
POST /subscriptions/api/resync/<slug>   → same body as /api/sync/<slug>, plus
...
2. **Claim.** mkvid polls `POST /mkvid/claim` (bearer `MKVID_TOKEN`) whenever its render slot is free and gets the next claimable request, or `null`. **Newest set first**: the queue is ordered by the set's date (`set_date`, from the `-YYYY-MM-DD.html` URL slug, else the page's `datePublished` / title), undated sets last, ties by most recently queued — so a DJ's latest show is uploaded before their 2014 back catalogue. The panel's mkvid section lists the waiting line in this order, and each row has **⤒ ↑ ↓ ⤓** to reorder it (`POST /subscriptions/api/mkvid/move/<id>` `{ "to": "top" | "up" | "down" | "bottom" }` — the order is a `sort_key` that defaults to the set date as a Julian day, so a moved row keeps its place while newly queued sets still slot in by date; `top` also outranks anything dated up to today) and **✕** to ban the set (`POST /subscriptions/api/mkvid/ban/<id>`: status `banned`, the row stays so the sync cannot queue it again; **Unban** in its details puts it back). A request whose set has meanwhile gained a real recording is marked `superseded` and skipped. A claim nobody reports on within `MKVID_CLAIM_TTL_SECONDS` (default 3 h — mkvid died mid-job) is handed out again; three claims and it is `failed`. **Two Google Cloud projects, two daily caps.** A `videos.insert` costs 1 600 of a project's 10 000 daily YouTube quota units (reset at midnight Pacific). mkvid has two OAuth clients for the same channel — `primary` in its own project **mkvid-uploads**, where nothing else spends, and `shared` in the sync's project **tracked-youtube**, which also pays 50 units for every playlist insert. The claim body says which accounts mkvid can currently upload with (`{ "accounts": ["primary", "shared"] }`; a body-less claim means primary only), the Worker fills the primary account first (`MKVID_DAILY_CLAIM_CAP`, default 6 = 9 600 units) and then spills to the shared one (`MKVID_SHARED_DAILY_CLAIM_CAP`, default 0 = never; 3 leaves the sync ~4 900 units), and the request it hands back names its `account`. Counts are `claimed`/`done` rows in `mkvid_requests` per account with `claimed_at` since midnight Pacific, so a deploy cannot reset them and a request refused before rendering does not use a slot. A cap of `0` **pauses** that account (every claim is refused) — it has to be a literal `0`; a blank value means the default. The panel shows both accounts' usage and says so in red when everything is paused.
...
The admin panel's **mkvid uploads** section opens with one status line that answers "why is nothing uploading?" — paused (cap 0), mkvid not polling (the Worker records each `/mkvid/claim` poll's outcome in KV, rewritten at most every 10 min), today's cap used (with the time until the midnight-Pacific reset), rendering, or ready — then the backlog estimate, what is rendering, the waiting line in claim order, and what finished. It lists every request (status, source, video, attempts, error, the privacy YouTube actually applied — an unverified OAuth app forces `private` even when `unlisted` was requested) with a **Retry** for failed ones. Endpoints (CF Access): `GET /subscriptions/api/mkvid`, `POST /subscriptions/api/mkvid/retry/<id>`. mkvid-side: `GET /mkvid/health` (counts, verifies the token), `POST /mkvid/job` (attach its job id).
...
1. **Home proxy** (free) — the residential-IP forwarder on the NAS (below). It tries the residential IP first; when 1001tracklists blocks that IP it reroutes the same request through a random tinyproxy bucket on the tailnet (18 distinct IPv4 egresses; a 19th, vm4:18181, is left out because its IP no longer routes) and reports what it did in `X-Proxy-*` headers. Tried when `HOME_PROXY_URL` + `HOME_PROXY_TOKEN` are set.
...
**Pacing.** 1001tracklists' block follows the login *session* (not the IP — see "What the block actually is" under Home proxy), and what trips it is per-account request rate. Every sync run therefore shares one budget of tracklist-page fetches across all subscriptions: `TL_FETCHES_PER_TICK` (default 20) **per account the forwarder reports healthy**, read from its `/status` once per tick. Three healthy accounts → 60 fetches a tick; one of them parked after a block → 40, automatically; forwarder unreachable or pre-0.4.0 → treated as one account. Subs are visited least-recently-run first, and when the budget is spent the rest wait for the next tick. The panel's manual buttons (`Sync`, `Invalidate & resync`, and both `…all` variants) run under the same budget (`manualFetchBudget`); until 2026-09-14 they did not, and the per-row loop behind "resync all" is what tripped both bans. The unpaced version fetched ~360 pages in 11 minutes from one account and got it banned on 2026-09-10; 25 per tick per account has been fine since.
...
- **A forwarder transport blip is a plain retry.** The forwarder's own 502 (`direct:error`: the home link could not reach 1001tracklists at all) throws `UpstreamTransportError` without consulting Bright Data; the set is charged one ordinary failure and the next tick tries again.
...
- **A dead YouTube video settles on the first strike.** When the set page names a video but YouTube rejects the insert as permanently unavailable (400/403/404 that is not a quota error: deleted, private, region-blocked), the set is recorded as `no_youtube` with the rejected id and marked processed immediately; the id is kept so the 5-day recheck still notices a replacement recording.
...
> **What the block actually is (measured 2026-09-10).** It arrives as HTTP 403 *or 429* with the `unblock_ip` form, and the flag attaches to the logged-in **session**, not the IP and not the account: the banned session was blocked from every egress; a fresh login made through a clean pool egress worked from everywhere, including the home IP; a fresh login made from the just-flagged home IP was born blocked (for ~25 minutes). Anonymous requests from any IP only ever see the ordinary Turnstile gate. So rotating egress IPs with the same cookie can never help — a fresh login through a clean egress does. The forwarder does exactly that (below); pacing (`TL_FETCHES_PER_TICK`) is what keeps the session from being flagged in the first place. The banner/push wording below still talks about "the home IP" because that is what the block page shows; the action it asks for (solve the captcha while logged in) is right either way.


`src/lib/ban-state.ts` turns the forwarder's `X-Proxy-*` report into KV state (`ban:home`, `ban:pause`, `ban:ep:*`, `ban:bd:<day>`) so that:

- every admin page (`/subscriptions`, `/subscriptions/tracklist`, `/subscriptions/dj/<slug>`) shows a big red banner while the home IP is blocked — with the blocked IP, the fallback pool's health, the next hourly probe time, a link to the captcha and an **"I solved it — re-probe now"** button that asks the forwarder to try the residential IP immediately, logging every parked account in fresh (the block sticks to the forwarder's session cookies, not to anything a browser can solve);
...
- **Accounts (0.4.0).** The forwarder holds any number of 1001tl accounts, one per numbered env pair `UPSTREAM_1001TL_EMAIL_<n>` / `UPSTREAM_1001TL_PASSWORD_<n>` (n = 1, 2, 3, …; discovered by listing the env), each with its own session persisted to `COOKIE_DIR/1001tl-cookies-<email>.json`. Requests go to the least-recently-used healthy account (round-robin, so each carries ~1/N of the load); a request that hits the block page on one account is retried on the same route with the next account (`account.failover`), and the blocked account is parked for `BLOCK_COOLDOWN_MS`. A parked account's fresh session is what gets tried first (below); only a fresh session that is *also* blocked says anything about the route. Response headers `x-proxy-account` (who served it) and `x-proxy-accounts-healthy` / `-total` feed the Worker's pacing; `/status` lists every account with counters. The real credentials live in `.1001tl-accounts.env` at the repo root (gitignored) and in the TrueNAS app config; `deploy.sh --set UPSTREAM_1001TL_EMAIL_4=… --set UPSTREAM_1001TL_PASSWORD_4=…` adds one.
- **Session re-issue (0.3.2, per account since 0.4.0).** When a route answers with the block page and the forwarder is in logged-in mode, it re-logins through a random healthy pool member (a clean egress issues a clean session; the flagged home IP issues a pre-blocked one) and retries the same request on the same route once. At most one re-login per `RELOGIN_COOLDOWN_MS` (default 10 min): a fresh session blocked again that fast means the account is genuinely being rate-limited, and the normal cooldown/pause path takes over. Successful recoveries carry `x-proxy-session-reissued: 1` and `x-proxy-block-scope: session`; an all-blocked answer carries `x-proxy-block-scope: account` when even the fresh session was blocked. Counters in `/status` → `relogin`.
- **Re-login walks the pool (0.4.3).** The login behind a re-issue goes through a random healthy pool member, with the same `REQUEST_TIMEOUT_MS` as a forwarded fetch on each of its two requests (before 0.4.3 a member that accepted the CONNECT and then stalled held the login, and every request waiting on that account's session, for undici's five-minute default); if that member fails in transport (asleep VM, dead CONNECT, timeout) the member is benched like it would be for a forwarded request and the next one is tried, up to `RELOGIN_LOGIN_ATTEMPTS` (default 3). Only a login 1001tracklists answered and rejected stops early. When no fresh session could be made at all, the account is benched for `ERROR_COOLDOWN_MS` (`login_failed`, 10 min) rather than parked for the hour-long block cooldown — an unreachable bucket says nothing about the account. Re-login outcomes are `recovered` / `still_blocked` / `failed` / `retry_error` (session issued, retry died in transport).
- **Probe heals the pool (0.4.1).** `POST /probe` (the hourly cron probe and the "I solved it — re-probe now" button) fetches the probe page as the least-recently-used account and, on a block, forces a fresh login *ignoring* `RELOGIN_COOLDOWN_MS` — a probe that reuses the cookie that was blocked in the first place can never learn anything, and until 0.4.1 that is exactly what the button did for ten minutes after every block, re-parking the account for another hour each click. Every other parked account gets the same forced fresh login in parallel, so one probe brings the whole pool back instead of leaving accounts parked on dead cookies for the rest of their cooldown (the Worker paces fetches by healthy accounts). The response's `probe` is the route's verdict (`ok` when any account got through); `relogin` is the primary account's re-login outcome (`recovered` / `still_blocked` / `failed`) and `healed[]` the others', and the banner footer spells those out. Solving the captcha in a browser does not touch the forwarder's session cookies at all — pressing re-probe is what heals them.
...
1. `scripts/nas-fetch-proxy-deploy/deploy.sh [--set KEY=VALUE ...]` copies the forwarder into the app's build dir, builds a fresh image tagged with `scripts/nas-fetch-proxy-deploy/package.json`'s version, patches the app's compose (image tag + any `--set` env upserts) through `midclt call app.update`, and starts the app. Needs `ssh mnmserver` with passwordless sudo.
   Env knobs: `PORT` (default 8088), `BIND` (default 0.0.0.0), `ALLOWED_HOSTS` (default `www.1001tracklists.com,1001tracklists.com`), `REQUEST_TIMEOUT_MS` (per attempt, default 15000), `FALLBACK_PROXIES` (comma/newline-separated `label=http://host:port` HTTP proxies — the tailnet tinyproxy buckets), `BLOCK_COOLDOWN_MS` (default 3600000), `MAX_POOL_ATTEMPTS` (default 2), `PROBE_URL` (tracklist page for `/probe`), `UPSTREAM_1001TL_EMAIL_<n>`/`UPSTREAM_1001TL_PASSWORD_<n>` (one pair per account, any count; none = anonymous mode), `RELOGIN_COOLDOWN_MS` (default 600000), `COOKIE_DIR` (default `/data` — persist on a volume so restarts don't re-login; a pre-0.4.0 `COOKIE_FILE` is adopted for the first account on first start).
